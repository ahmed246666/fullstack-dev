...
export const api = {
  ...
  getCustomers: (params?: { search?: string; tier?: string; page?: number; limit?: number }) => {
    ...
    return fetchApi<{ success: boolean; data: any[]; pagination: any }>(`/customers?${searchParams.toString()}`);
  },
  
  getCustomerById: (id: string) => fetchApi<{ success: boolean; data: any }>(`/customers/${id}`),
  
  createCustomer: (data: any) => fetchApi<{ success: boolean; data: any }>('/customers', {
    method: 'POST',
    body: JSON.stringify(data)
  }),

  // Tickets
  getTickets: (params?: { search?: string; status?: string; priority?: string; channel?: string; page?: number; limit?: number }) => {
    const searchParams = new URLSearchParams();
    if (params?.search) searchParams.set('search', params.search);
    ...
    if (params?.page) searchParams.set('page', String(params.page));
    if (params?.limit) searchParams.set('limit', String(params.limit));
    return fetchApi<{ success: boolean; data: any[]; pagination: any }>(`/tickets?${searchParams.toString()}`);
  },

  getTicketById: (id: string) => fetchApi<{ success: boolean; data: any }>(`/tickets/${id}`),

  createTicket: (data: any) => fetchApi<{ success: boolean; data: any }>('/tickets', {
    method: 'POST',
    body: JSON.stringify(data)
  }),

  updateTicketStatus: (id: string, status: string, actorName?: string) => fetchApi<{ success: boolean; data: any }>(`/tickets/${id}/status`, {
    method: 'PATCH',
    body: JSON.stringify({ status, actorName })
  }),

  addTicketNote: (ticketId: string, data: { content: string; authorName?: string; isInternal?: boolean; channel?: string }) => 
  ...
  getKnowledgeArticles: (params?: { search?: string; category?: string }) => {
    ...
    return fetchApi<{ success: boolean; data: any[] }>(`/knowledge-base?${searchParams.toString()}`);
  },
