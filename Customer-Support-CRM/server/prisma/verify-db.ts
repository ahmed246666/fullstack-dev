...
async function runTests() {
  ...
  console.log(
    `✅ [1/7] Users count: ${users.length} (Admin: ${users.filter((u) => u.role === 'ADMIN').length}, Agents: ${users.filter((u) => u.role === 'AGENT').length})`
  );
  ...
  customers.forEach((c) =>
    console.log(`   - ${c.name} (${c.nameAr}) [Tier: ${c.tier}] -> ${c.tickets.length} tickets`)
  );
  ...
  tickets.forEach((t) => {
    ...
    console.log(
      `     Channel: ${t.channel} | Priority: ${t.priority} | Status: ${t.status} | Agent: ${t.assignedAgent?.name || 'Unassigned'} | Notes: ${t.notes.length}`
    );
  });
  ...
  slas.forEach((s) =>
    console.log(
      `   - Priority: ${s.priority} -> Response: ${s.responseTimeHours}h, Resolution: ${s.resolutionTimeHours}h`
    )
  );
  ...
  articles.forEach((a) =>
    console.log(
      `   - [${a.slug}] ${a.title} / ${a.titleAr} (Votes: +${a.helpfulVotes}/-${a.unhelpfulVotes})`
    )
  );
  ...
  audits.forEach((a) =>
    console.log(`   - Action: ${a.action} on ${a.entity} by ${a.actor?.name || 'System'}`)
  );
